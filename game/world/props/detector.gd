...
const S := 0.8  # sprite scale (540x320 art)
...
func _setup() -> void:
	...
	make_sprite("res://assets/sprites/detector.svg", S, Vector2(0, -(282.0 - 160.0) * S))
...
func _draw_dynamic(c: Node2D) -> void:
	var base_y := -(282.0 - 160.0) * S
	var font := ThemeDB.fallback_font
	# Sign text on the white plate (plate center: doc (270,152) -> local).
	var sign_text := "MUON OBSERVATORY"
	var w := font.get_string_size(sign_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 17).x
	c.draw_string(font, Vector2(-w * 0.5, base_y + (152.0 - 160.0) * S + 6.0),
		sign_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 17, Juice.INK)
	...
	c.draw_circle(Vector2((110.0 - 270.0) * S, base_y + (150.0 - 160.0) * S), 6.5, led)
	if _celebrate > 0.0:
		# Above the roofline, where the muon can't photobomb it.
		c.draw_string(Juice.hand_font, Vector2(-42.0, base_y + (94.0 - 160.0) * S - 10.0),
			"+1 muon!!", HORIZONTAL_ALIGNMENT_LEFT, -1, 20, Color("2e8b57"))
