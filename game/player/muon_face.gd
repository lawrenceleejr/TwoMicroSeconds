...
func _draw() -> void:
	...
	draw_circle(Vector2(9.5, 3.0), 3.6, Color(Juice.BLUSH, 0.85))
	# Eyes: blink by squashing, widen with speed (someone is having fun).
	var widen := 1.0
	if muon_node != null:
		var vel2: Vector2 = muon_node.get("velocity") if muon_node.get("velocity") != null else Vector2.ZERO
		widen = 1.0 + 0.45 * clampf(vel2.length() / 2600.0, 0.0, 1.0)
	for side in [-1.0, 1.0]:
		var eye_pos := Vector2(side * 5.5, -3.0) + look
		draw_set_transform(eye_pos, 0.0, Vector2(1.0, _blink))
		draw_circle(Vector2.ZERO, 2.7 * widen, Juice.INK)
